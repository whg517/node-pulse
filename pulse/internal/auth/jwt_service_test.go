...
func TestJWTService_ValidateAccessToken_Expired(t *testing.T) {
	...
	// Create service with 0 expiration for immediate expiry
	service := NewJWTService("test-secret-key-32-bytes-1234567890", 0, pool)
...
func setupTestDBWithCleanup(t *testing.T) *pgxpool.Pool {
	// Use existing test utilities from internal/db/test_utils
	// For now, skip tests if DB not available
	t.Skip("JWT service tests require test database - to be implemented with internal/db/test_utils.setupTestDB")
	return nil
}
