...
func TestCacheConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid config",
			config:  CacheConfig{Enabled: true, MaxSizeBytes: 10 * 1024 * 1024},
			wantErr: false,
		},
		{
			name:    "negative max size",
			config:  CacheConfig{MaxSizeBytes: -1},
			wantErr: true,
		},
		{
			name:    "max size too large",
			config:  CacheConfig{MaxSizeBytes: 200 * 1024 * 1024},
			wantErr: true,
		},
	}
...
func TestDefaultConfigs(t *testing.T) {
	// Test DefaultModeConfig
	modeCfg := DefaultModeConfig()
	if modeCfg.Mode != ModeRegistered {
		t.Errorf("Expected default mode %s, got %s", ModeRegistered, modeCfg.Mode)
	}
	if modeCfg.ConfigCheckIntervalSeconds != 60 {
		t.Errorf("Expected default interval 60, got %d", modeCfg.ConfigCheckIntervalSeconds)
	}

	// Test DefaultCompressionConfig
	compCfg := DefaultCompressionConfig()
	if compCfg.Level != 6 {
		t.Errorf("Expected default compression level 6, got %d", compCfg.Level)
	}

	// Test DefaultCacheConfig
	cacheCfg := DefaultCacheConfig()
	if cacheCfg.MaxSizeBytes != 10*1024*1024 {
		t.Errorf("Expected default cache size 10MB, got %d", cacheCfg.MaxSizeBytes)
	}
}
