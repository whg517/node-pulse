...
### 6.4 API 规范 (API Specification)

Server 应提供 RESTful API 用于 Beacon 管理、数据查询和系统配置。
...
**GET /api/beacons/{id}/config/history**
...
**查询参数:**
- `beacon_id`: string (必需)
- `start_time`: timestamp (必需)
- `end_time`: timestamp (必需)
- `metrics`: string (可选,逗号分隔,默认 "latency,loss,jitter")
...
{
  "error": "unauthorized",
  "message": "Missing or invalid Node-Token"
}
```
